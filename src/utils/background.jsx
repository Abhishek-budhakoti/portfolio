...
const Background = ({ children, theme }) => {
    ...
    const handleMouseMove = (e) => {
        const deltaX = e.clientX - motion.current.lastX;
        const deltaY = e.clientY - motion.current.lastY;
        motion.current.vx = deltaX;
        motion.current.vy = deltaY;
        motion.current.lastX = e.clientX;
        motion.current.lastY = e.clientY;
    };

    return (
        <div
            onMouseMove={handleMouseMove}
            className={`relative w-full h-screen overflow-hidden ${isDark
                ? "bg-slate-950"
        ...
        >
            ...
            <div
                className={`relative z-10 w-full h-full overflow-auto ${isDark ? "text-white" : "text-black"
                    }`}
            >
                {children}
            </div>
